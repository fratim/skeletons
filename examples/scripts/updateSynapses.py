...
prefix = 'Zebrafinch'
max_label = 410
block_size = [256, 2048, 2048]
volume_size = [256*23,2048*3,2048*3]
...
for label in range(1, max_label):
    # read all synapses
    synapse_filename = 'synapses_raw/{}/syn_{:04d}.txt'.format(prefix, label)
...
for zblock in range(nzblocks):
    for yblock in range(nyblocks):
        for xblock in range(nxblocks):
            ...
            synapse_filename = 'synapses_2048x2048x256/Zebrafinch/{}-synapses-{:04d}z-{:04d}y-{:04d}x.pts'.format(prefix, zblock, yblock, xblock)
